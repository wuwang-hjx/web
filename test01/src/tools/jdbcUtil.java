...
//		result = stm.executeQuery(sql);
	...
	public int createuser(String user,String pass,String name,String email,int sf,int cs) throws ClassNotFoundException, SQLException {
		...
		con = DriverManager.getConnection(dburl,dbuser,dbpass);
		stm = con.createStatement();
		ResultSet st = stm.executeQuery("select userName from t_user where userName=\""+user+"\";");
		if(!st.next()) {
			String sql = "insert into t_user(`userName`,`password`,`charName`,`email`,`sf`,`cs`) values(\""+user+"\",\""+pass+"\",\""+name+"\",\""+email+"\",\""+sf+"\",\""+cs+"\");";
			//System.out.println(sql);
			int it = stm.executeUpdate(sql);
			if(it > 0) {
				result=0;
			}
		}else {
